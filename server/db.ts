...
export async function getDb() {
  if (!_db) {
    // Support both DATABASE_URL (standard) and MYSQL_URL (Railway)
    const dbUrl = process.env.DATABASE_URL || process.env.MYSQL_URL;
    
    if (dbUrl) {
      try {
        console.log("[Database] Connecting to:", dbUrl.substring(0, 50) + "...");
        _db = drizzle(dbUrl);
      ...
      console.warn("[Database] No DATABASE_URL or MYSQL_URL found in environment");
    }
  }
  return _db;
}
